#!/bin/bash
########################################################################################################################
# Run all FairFace experiments: 3 backbones × 4 model types
#
# For each backbone:
...
run_backbone() {
    ...
    # Read the best lambda (cross_val.py saves it to a text file)
    # Build the suffix the same way cross_val.py does
    ...
    for MODEL_TYPE in fair baseline penalty strict_penalty; do
        ...
        # Build the command
        CMD="python run_fairface_experiments.py \
...
echo ""
echo "╔══════════════════════════════════════════════════════════════╗"
echo "║        FairFace Experiments — Full Ablation Suite           ║"
echo "╚══════════════════════════════════════════════════════════════╝"
...
for r in rows:
    ...
    bb = r.get('backbone', '?')
    # Handle both baseline (loss_proj) and other (loss) formats
