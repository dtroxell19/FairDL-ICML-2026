########################################################################################################################
# F-Layer (fairness layer) model for the CelebA experiments.
#
# The fairness layer projects logits onto the pairwise demographic parity constraint
# set for all C(4,2) = 6 intersectional group pairs via a differentiable cvxpylayer.
########################################################################################################################
...
# Tighter SCS tolerances to ensure per-batch constraint satisfaction.
# Default SCS eps (~1e-5) allows constraint violations that accumulate
# across batches; eps=1e-8 keeps violations well below ε=0.0001.
SCS_SOLVER_ARGS = {"eps": 1e-8, "max_iters": 10000, "acceleration_lookback": 0}


class FairModel(BaselineModel):
    """
    F-Layer model: end-to-end trainable fairness via a differentiable optimization layer
    enforcing pairwise demographic parity across 4 intersectional groups.
    """
    ...
    def create_fair_layer(self, n, group_masks_np):
        ...
        @param group_masks_np: dict {group_id: (mask_array, count)}
    ...
    def _build_group_masks(self, groups_np):
        """Build dict of {group_id: (float_mask, count)} for groups present in batch."""
        masks = {}
    ...
    def _solve_fair_plain_cvxpy(self, raw_np, group_masks_np, epsilon):
        """
        Plain CVXPY fallback for hard-constraint projection (no CvxpyLayer).
        Used when inference=True and CvxpyLayer/diffcp fails.
        ...
        problem.solve(solver=cp.SCS, eps=1e-8, max_iters=10000, acceleration_lookback=0, verbose=False)
    ...
    def _solve_primal_dual_plain_cvxpy(self, raw_np, group_masks_np, lambda_dual, batch_size):
        """
        Plain CVXPY fallback for primal-dual projection (no CvxpyLayer).
        Used when inference=True and CvxpyLayer/diffcp fails.
    ...
    def forward(self, images, groups, inference=False):
        ...
        @param groups (Tensor): (B,) integer intersectional group IDs
        ...
        # Need at least 2 groups to form constraints
        ...
        if n < b_tau:
            ...
            try:
                ...
                # Pass solver_args if your version uses them:
                try:
                    (ytilde,) = layer(raw_cpu, slack_tensor, solver_args=SCS_SOLVER_ARGS)
                except NameError:
                    (ytilde,) = layer(raw_cpu, slack_tensor)
            except Exception:
                if inference:
                    # Fallback to plain CVXPY
    ...
    #     @param images (Tensor): (B, 3, H, W)
    #     @param groups (Tensor): (B,) integer intersectional group IDs (0–3)
    #     @param inference (bool): if True, use inference dual variable
    ...
    #     # cvxpylayers solves on CPU in float32
    #     raw_cpu = raw.cpu().float()
    #     epsilon = xp_params.get_slack()
    #     b_tau = xp_params.b_tau()
