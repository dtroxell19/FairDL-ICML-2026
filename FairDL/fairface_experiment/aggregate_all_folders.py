########################################################################################################################
# Aggregate FairFace results across multiple results folders and produce:
...
#   python aggregate_all_folders.py --dirs results results_2models results_pt1
#   python aggregate_all_folders.py --dirs results results_2models results_pt1 --output combined_results
########################################################################################################################
...
def generate_markdown_table(data):
    ...
    lines = []
    lines.append("**FairFace test set results across architectures and fairness methods.** "
    ...
    for bb in BACKBONE_ORDER:
        ...
        for j, mt in enumerate(METHOD_ORDER):
            ...
            if mt == "fair":
                ...
                delta_pct = (acc - fair_acc) / fair_acc * 100
                delta_str = f"{delta_pct:+.2f}"
            else:
                delta_str = "—"
...
def main():
    ...
    md_path = output_dir / "fairface_results_table.md"
...
def get_args():
    ap = argparse.ArgumentParser(
        description="Aggregate FairFace results across multiple folders and produce markdown table."
    )
