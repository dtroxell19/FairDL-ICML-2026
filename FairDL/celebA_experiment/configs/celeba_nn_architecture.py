########################################################################################################################
# Neural network architectures for the CelebA experiments.
#
# Identical backbone registry as FairFace — the architecture is task-agnostic.
# Supports the same 5 architecture families for ablation studies:
...
def apply_lora(model, rank=8, alpha=16):
    """Apply LoRA adapters to MLP Linear layers in a ViT/Swin encoder."""
...
def get_backbone(name="resnet18", pretrained=True, freeze=False, lora=False,
    ...
    @param lora (bool): apply LoRA adapters (ViT/Swin only)
...
def get_classifier_head(num_features, head_type="linear"):
    """
    Classification head: maps (B, num_features) → (B, 1) logit.
